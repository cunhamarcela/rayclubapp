...
class SupabaseRedeemedBenefitRepository implements RedeemedBenefitRepository {
  ...
  Future<List<RedeemedBenefit>> getUserRedeemedBenefits() async {
    try {
      ...
      // Primeiro, atualizar benefícios expirados
      await _updateExpiredBenefits(userId);
      
      // Buscar todos os benefícios do usuário
      ...
      return response
          .map((json) => _mapResponseToBenefit(json))
          .toList();
    } catch (e) {
      if (e is AppAuthException) rethrow;
      
      // Em desenvolvimento, retornar dados mockados em caso de erro
      return MockRedeemedBenefitRepository().getUserRedeemedBenefits();
    }
  }
...
final redeemedBenefitRepositoryProvider = Provider<RedeemedBenefitRepository>((ref) {
  ...
  return MockRedeemedBenefitRepository();
